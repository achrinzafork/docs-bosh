...
- **converge\_variables** [Boolean, optional]: Enable variables to be regenerated by the backend config server (e.g. [CredHub](https://docs.cloudfoundry.org/api/credhub/version/main/#_overwriting_credential_values)) when the variable `options` change. Default `false`. Available in bosh-release 267+.
...
- **env** [Hash, optional]: Specifies advanced BOSH Agent configuration for each instance in the group. [Read more about the agent.](agent-cpi-interactions.md#agent-settings-format)
    ...
    - **persistent_disk_mount_options** [Array of strings, optional]: Mount
      options when mounting persistent disk. Example: `["noatime"]`.
    - **persistent_disk_partitioner** [String, optional]: Supported values:
      empty string (`parted` for ephemeral; `sfdisk` for persistent if < 2TB
      otheriwse `parted`), `sfdisk` and `parted`. Default: empty string.
    - **bosh** [Hash, optional]:
        ...
        * **keep\_root\_password** [Boolean, optional]: Keep password for `root` and only change password for `vcap`. Default: `false`.
        ...
        - **agent** [Hash, optional]:
            - **settings** [Hash, optional]:
                - **tmpfs** [Boolean, optional]: Mount agent settings on a
                  [tmpfs](creds-tmpfs.md) directory. Default: `false`.
        - **blobstores** [Array, optional]: Blobstore configuration.
            - **provider** [String, requried]: Blobstore provider to use.
            - **options** [Hash, optional]: Provider-specific blobstore
              configuration.
        - **ntp** [Array, optional]: list of [ntp servers to synchronize
          to](ntp-config.md). Default: director-configured ntp servers.
        - **parallel** [Integer, optional]: Maximum number of jobs and
          packages to download in parallel. Default: `5`.
